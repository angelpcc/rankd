...
export default function Hero() {
  ...
  return (
    <section id="home" style={{ position: 'relative', width: '100%', minHeight: '100vh', display: 'flex', alignItems: 'center', overflow: 'hidden', background: '#030303' }}>
      ...
      {/* BG — peleador en saco, plano medio, sin cara */}
      <div ref={bgRef} style={{ position: 'absolute', inset: 0, zIndex: 0, opacity: 0, transform: 'scale(1.04)', transition: 'all 1.4s ease' }}>
        <img
          src="https://images.unsplash.com/photo-1517438322307-e67111335449?w=1920&q=95&fit=crop&crop=center"
          alt=""
          style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: '60% center' }}
        />
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(90deg, rgba(3,3,3,1) 0%, rgba(3,3,3,0.93) 42%, rgba(3,3,3,0.55) 68%, rgba(3,3,3,0.08) 100%)' }} />
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }} className="hero-right">
          {[
            ...
            <div key={card.title} style={{ padding: '20px 22px', borderRadius: 16, background: 'rgba(255,255,255,0.04)', border: `1px solid ${card.color}22`, backdropFilter: 'blur(16px)', display: 'flex', alignItems: 'center', gap: 16, transition: 'all 0.25s', cursor: 'pointer' }}
              ...
              onMouseLeave={(e) => { (e.currentTarget as HTMLDivElement).style.background = 'rgba(255,255,255,0.04)'; (e.currentTarget as HTMLDivElement).style.transform = 'none'; (e.currentTarget as HTMLDivElement).style.borderColor = `${card.color}22`; }}>
          ...
          <div style={{ padding: '14px 20px', borderRadius: 12, background: 'rgba(34,197,94,0.05)', border: '1px solid rgba(34,197,94,0.15)', display: 'flex', alignItems: 'center', gap: 10 }}>
            <span style={{ width: 7, height: 7, background: '#22c55e', borderRadius: '50%', boxShadow: '0 0 6px #22c55e', flexShrink: 0 }} />
            <span style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 13, color: 'rgba(255,255,255,0.35)', letterSpacing: 1 }}>Plataforma activa · Únete gratis</span>
          </div>
        </div>
      </div>
