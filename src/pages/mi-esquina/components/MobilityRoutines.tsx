...
import {
  MOBILITY_ROUTINES, mobilityDoneToday, toggleMobilityDone, routineSeconds, mobilityHow,
...
export default function MobilityRoutines() {
  ...
  // Qué explicación de movimiento está abierta ("<zona>:<índice>"). Solo una a
  // la vez, para que la lista no se vuelva un muro de texto.
            ...
            <div key={r.id} className="rk-card" style={{ padding: 0, overflow: 'hidden' }}>
              ...
              {isOpen && (
                <div className="px-4 pb-4 border-t border-white/[0.06] pt-3 space-y-2">
                  {r.moves.map((m, i) => {
                    const how = mobilityHow(m.name.en, lang);
                    const showHow = howOpen === `${r.id}:${i}`;
                    return (
                      <div key={i} className="flex items-start gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {/* El `cue` es un recordatorio de una línea. Quien no
                              conoce el movimiento necesita la explicación
                              entera, así que se puede desplegar aquí mismo. */}
                          {how && (
                            <>
                              <button type="button" onClick={() => setHowOpen(showHow ? null : `${r.id}:${i}`)}
                                aria-expanded={showHow} style={{ minHeight: 32 }}
                                className="mt-1 inline-flex items-center gap-1 text-[11px] font-semibold text-zinc-400 hover:text-white cursor-pointer">
                                <i className={showHow ? 'ri-arrow-up-s-line' : 'ri-information-line'} />
                                {t('mc_mob_how')}
                              </button>
                              {showHow && (
