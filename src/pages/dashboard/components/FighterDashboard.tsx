...
export default function FighterDashboard({ profile }: Props) {
  ...
  // Profile form state — mirrors both profiles + fighters tables
  ...
  // Populate form from fighter data
  ...
  const publishProfile = async () => {
    if (!fighter) return;
    // Only validate when trying to make public
    if (!isPublic) {
      const check = canPublish();
      if (!check.ok) {
        showToast(check.reason || t('dash_fighter_validation_incomplete'), 'error');
  ...
  const saveProfile = async () => {
    ...
    try {
      ...
      const fighterPayload = {
        ...
        kos: parseInt(kos, 10) || 0,
        experience_level: expLevel || null,
        gym: gym.trim(),
        coach: coach.trim(),
        looking_for: lookingFor,
        highlight_video: highlightVideo.trim(),
      ...
      if (fighter) {
        ...
        if (updatedFighter) {
          ...
          populateFromFighter(updatedFighter);
        }
      } else {
        const { data: newFighter, error: insertError } = await supabase
          .from('fighters')
          .insert(fighterPayload)
          .select()
          .maybeSingle();
        ...
        if (newFighter) {
          ...
          populateFromFighter(newFighter);
        }
      }
  ...
  const addVideo = async () => {
    ...
    const { data, error } = await supabase
      .from('fighter_videos')
      .insert({ fighter_id: fighter.id, title: videoTitle.trim(), url: videoUrl.trim(), video_type: videoType })
      .select()
      .maybeSingle();
  ...
  const addAchievement = async () => {
    ...
    const { data, error } = await supabase
      .from('fighter_achievements')
      .insert({
        fighter_id: fighter.id,
        title: achTitle.trim(),
        year: achYear ? parseInt(achYear, 10) : null,
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white">
      ...
      <div className="pt-16 max-w-6xl mx-auto px-4 md:px-6 py-8">
          ...
          <div className="flex items-center gap-4 w-full sm:w-auto">
            <div className="flex-shrink-0">
              <AvatarUpload
                ...
                displayName={fullName || profile.full_name || 'F'}
                onUploadSuccess={(url) => {
                  setAvatarUrl(url);
          ...
          <div className="flex items-center gap-2 flex-wrap sm:ml-auto">
            ...
            {fighter && (
              <button
                ...
                disabled={publishing}
                className={`flex items-center gap-1.5 text-xs px-3 py-2 rounded-xl font-semibold border transition-all cursor-pointer whitespace-nowrap disabled:opacity-60 ${
                  isPublic
                    ? 'bg-green-500/15 border-green-500/40 text-green-400 hover:bg-red-500/15 hover:border-red-500/40 hover:text-red-400'
                    : 'bg-zinc-800 border-zinc-600 text-zinc-300 hover:bg-green-500/15 hover:border-green-500/40 hover:text-green-400'
                }`}
              >
                {publishing
                  ? <><div className="w-3 h-3 border border-current border-t-transparent rounded-full animate-spin"></div> {t('dash_saving')}</>
                  : isPublic
                  ? <><i className="ri-eye-line"></i><span className="hidden sm:inline">{t('dash_fighter_visible')}</span><span className="sm:hidden">{t('dash_fighter_public')}</span></>
                  : <><i className="ri-eye-off-line"></i><span className="hidden sm:inline">{t('dash_fighter_make_public')}</span><span className="sm:hidden">{t('dash_fighter_publish')}</span></>
                }
              </button>
            )}
            {fighter && isPublic && (
              <>
                <Link
                  ...
                  className="flex items-center gap-1.5 text-xs px-3 py-2 rounded-xl bg-red-600 hover:bg-red-700 text-white font-medium transition-colors cursor-pointer whitespace-nowrap"
                >
                  <i className="ri-external-link-line"></i>
                  <span className="hidden sm:inline">{t('dash_fighter_view_public')}</span>
                  <span className="sm:hidden">{t('dash_fighter_view_profile')}</span>
                </Link>
                <button
                  onClick={() => {
                    navigator.clipboard.writeText(`${window.location.origin}/fighter/${fighter.id}`);
                    showToast(t('dash_settings_copied'));
                  }}
                ...
                >
                  ...
                  <span className="hidden sm:inline">{t('dash_fighter_share')}</span>
                </button>
              </>
            )}
        ...
        {/* Private profile banner — prominent */}
        {fighter && !isPublic && (
            ...
            <div className="bg-amber-500/15 px-5 py-4 flex flex-col sm:flex-row items-start sm:items-center gap-4">
              ...
              >
                {publishing
                  ...
                  : <><i className="ri-eye-line text-base"></i> {t('dash_fighter_private_cta')}</>
                }
              </button>
            </div>
          </div>
        ...
        {activeTab === 'profile' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full bg-zinc-800 border border-zinc-700 text-white text-sm rounded-xl px-4 py-2.5 focus:outline-none focus:border-red-500"
                  placeholder="El Toro, The Machine..."
                />
              </div>
                  ...
                  {[
                    ...
                    <div key={s.label}>
                      ...
                      <input
                        ...
                        className="w-full bg-zinc-800 border border-zinc-700 text-white text-sm rounded-xl px-3 py-2.5 focus:outline-none focus:border-red-500 text-center"
                      />
                    </div>
                  ))}
                </div>
            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 space-y-4">
              ...
              <div className="flex items-center justify-between pt-2">
                ...
                <button
                  ...
                  className={`relative w-12 h-6 rounded-full transition-colors cursor-pointer flex-shrink-0 ${isAvailable ? 'bg-red-600' : 'bg-zinc-700'}`}
                >
            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 space-y-4">
              ...
              <p className="text-xs text-zinc-500 -mt-2">{t('dash_digital_desc')}</p>

              {/* Preview badges */}
              {(instagram || tiktok || youtube || twitter) && (
                <div className="flex flex-wrap gap-2 py-2 border-t border-zinc-800">
                  {instagram && (
                    <span className="flex items-center gap-1.5 text-xs bg-gradient-to-r from-pink-600/20 to-orange-600/20 border border-pink-500/30 text-pink-400 px-3 py-1.5 rounded-full">
                      <i className="ri-instagram-line"></i> {instagram}
                    </span>
                  )}
                  {tiktok && (
                    <span className="flex items-center gap-1.5 text-xs bg-zinc-800 border border-zinc-600 text-zinc-300 px-3 py-1.5 rounded-full">
                      <i className="ri-tiktok-line"></i> {tiktok}
                    </span>
                  )}
                  {youtube && (
                    <span className="flex items-center gap-1.5 text-xs bg-red-600/20 border border-red-500/30 text-red-400 px-3 py-1.5 rounded-full">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="flex items-center gap-1.5 text-xs text-zinc-400 mb-1.5">
                    <span className="w-5 h-5 flex items-center justify-center rounded bg-gradient-to-br from-pink-500 to-orange-400 text-white text-xs"><i className="ri-instagram-line"></i></span>
                    Instagram
                  </label>
                  <input
                    value={instagram}
                ...
                <div>
                  <label className="flex items-center gap-1.5 text-xs text-zinc-400 mb-1.5">
                    <span className="w-5 h-5 flex items-center justify-center rounded bg-zinc-800 border border-zinc-600 text-white text-xs"><i className="ri-tiktok-line"></i></span>
                    TikTok
                  </label>
                  <input
                    value={tiktok}
                ...
                <div>
                  <label className="flex items-center gap-1.5 text-xs text-zinc-400 mb-1.5">
                    <span className="w-5 h-5 flex items-center justify-center rounded bg-red-600 text-white text-xs"><i className="ri-youtube-line"></i></span>
                    YouTube
                  </label>
                  <input
                    value={youtube}
                ...
                <div>
                  <label className="flex items-center gap-1.5 text-xs text-zinc-400 mb-1.5">
                    <span className="w-5 h-5 flex items-center justify-center rounded bg-zinc-900 border border-zinc-600 text-white text-xs"><i className="ri-twitter-x-line"></i></span>
                    Twitter / X
                  </label>
                  <input
                    value={twitter}
            ...
            <div className="lg:col-span-2">
              ...
              >
                {saving
                  ...
                  : <><i className="ri-save-line"></i> {t('dash_save_profile')}</>
                }
              </button>
              {fighter && (
          ...
          <div className="max-w-3xl">
            <div className="mb-6">
              <h2 className="text-xl font-bold text-white flex items-center gap-2">
                {t('dash_verification_title')}
                {currentProfile.verified && <i className="ri-shield-check-fill text-green-400"></i>}
              </h2>
        ...
        {activeTab === 'videos' && (
          <div className="space-y-6">
            {!fighter && (
              <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-xl p-4 text-sm text-yellow-400 flex items-center gap-2">
                <i className="ri-information-line"></i>
                {t('dash_video_save_first')}
              </div>
            )}
            <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <input
                  ...
                  className="bg-zinc-800 border border-zinc-700 text-white text-sm rounded-xl px-4 py-2.5 focus:outline-none focus:border-red-500"
                  placeholder={t('dash_video_title_ph')}
                  disabled={!fighter}
                />
                <input
                  value={videoUrl}
                  ...
                  className="bg-zinc-800 border border-zinc-700 text-white text-sm rounded-xl px-4 py-2.5 focus:outline-none focus:border-red-500"
                  placeholder={t('dash_video_url_ph')}
                  disabled={!fighter}
                />
                <div className="flex gap-2">
                  <select
                    ...
                    className="flex-1 bg-zinc-800 border border-zinc-700 text-white text-sm rounded-xl px-3 py-2.5 focus:outline-none focus:border-red-500 cursor-pointer"
                    disabled={!fighter}
                  >
                  ...
                  <button
                    onClick={addVideo}
                    disabled={!fighter || !videoTitle.trim() || !videoUrl.trim()}
                    className="bg-red-600 hover:bg-red-700 disabled:opacity-40 text-white px-4 rounded-xl transition-colors cursor-pointer whitespace-nowrap"
                  >
            ...
            {videos.length === 0 ? (
              <div className="text-center py-16 text-zinc-500">
                <div className="w-16 h-16 flex items-center justify-center mx-auto mb-4"><i className="ri-video-line text-4xl"></i></div>
                <p className="text-sm">{t('dash_video_empty')}</p>
              </div>
            ) : (
                  ...
                  <div key={v.id} className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden group">
                    ...
                    <div className="p-3 flex items-center justify-between">
                      <div className="min-w-0 flex-1">
                        ...
                        <a href={v.url} target="_blank" rel="nofollow noreferrer" className="text-xs text-red-400 hover:text-red-300 truncate block">{t('dash_video_watch')}</a>
                      </div>
                      <button
                        onClick={() => deleteVideo(v.id)}
                        className="w-8 h-8 flex items-center justify-center text-zinc-500 hover:text-red-400 cursor-pointer opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0"
                      >
        ...
        {activeTab === 'achievements' && (
          <div className="space-y-6">
            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <input
                  ...
                  className="bg-zinc-800 border border-zinc-700 text-white text-sm rounded-xl px-4 py-2.5 focus:outline-none focus:border-red-500"
                  placeholder={t('dash_ach_title_ph')}
                  disabled={!fighter}
                />
                <input
                  type="number"
                  ...
                  className="bg-zinc-800 border border-zinc-700 text-white text-sm rounded-xl px-4 py-2.5 focus:outline-none focus:border-red-500"
                  placeholder={t('dash_ach_year_ph')}
                  disabled={!fighter}
                />
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-zinc-800 border border-zinc-700 text-white text-sm rounded-xl px-4 py-2.5 focus:outline-none focus:border-red-500"
                    placeholder={t('dash_ach_desc_ph')}
                    disabled={!fighter}
                  />
                  <button
                    onClick={addAchievement}
                    disabled={!fighter || !achTitle.trim()}
                    className="bg-red-600 hover:bg-red-700 disabled:opacity-40 text-white px-4 rounded-xl transition-colors cursor-pointer whitespace-nowrap"
                  >
                    <i className="ri-add-line"></i>
                  </button>
            ...
            {achievements.length === 0 ? (
              <div className="text-center py-16 text-zinc-500">
                <div className="w-16 h-16 flex items-center justify-center mx-auto mb-4"><i className="ri-medal-line text-4xl"></i></div>
                <p className="text-sm">{t('dash_ach_empty')}</p>
              </div>
            ) : (
                ...
                {achievements.map((a) => (
                  <div key={a.id} className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 flex items-center gap-4 group">
                    ...
                    <button
                      ...
                      className="w-8 h-8 flex items-center justify-center text-zinc-500 hover:text-red-400 cursor-pointer opacity-0 group-hover:opacity-100 transition-opacity"
                    >
                      <i className="ri-delete-bin-line"></i>
                    </button>
                  </div>
          ...
          <div className="max-w-lg space-y-4">
            <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between py-3 border-b border-zinc-800">
                  <div>
                    <p className="text-sm text-white">{t('dash_settings_type')}</p>
                    <p className="text-xs text-zinc-500">{t('dash_fighter_label')}</p>
                  </div>
                ...
                <div className="flex items-center justify-between py-3 border-b border-zinc-800">
                  <div>
                    <p className="text-sm text-white">{t('dash_settings_availability')}</p>
                    <p className="text-xs text-zinc-500">{t('dash_settings_availability_desc')}</p>
                  </div>
                  <button
                    type="button"
                    onClick={() => setIsAvailable(!isAvailable)}
                    className={`relative w-12 h-6 rounded-full transition-colors cursor-pointer ${isAvailable ? 'bg-red-600' : 'bg-zinc-700'}`}
                  >
                    <span className={`absolute top-1 w-4 h-4 bg-white rounded-full transition-all ${isAvailable ? 'left-7' : 'left-1'}`}></span>
                  </button>
                </div>
                  ...
                  <div className="py-3">
                    ...
                    <div className="flex items-center gap-2 bg-zinc-800 border border-zinc-700 rounded-xl px-4 py-2.5">
                      ...
                      <button
                        onClick={() => {
                          navigator.clipboard.writeText(`${window.location.origin}/fighter/${fighter.id}`);
                          showToast(t('dash_settings_copied_short'));
                        }}
            ...
            {isAvailable !== (fighter?.is_available ?? true) && (
              <button
                onClick={saveProfile}
                disabled={saving}
                className="w-full bg-red-600 hover:bg-red-700 text-white font-semibold py-3 rounded-xl transition-colors cursor-pointer whitespace-nowrap disabled:opacity-60 flex items-center justify-center gap-2 text-sm"
              >
