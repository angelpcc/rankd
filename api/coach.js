    ...
    protocols: {
      ...
      items: {
        ...
        properties: {
          ...
          weekdays: { type: 'array', description: 'Días (0 = lunes) en los que toca este cardio. Lista VACÍA = no va a la agenda: se guarda en "Cardios guardados" para hacerlo el día que le apetezca. Úsalo para opciones alternativas ("el sábado elige una"), cardios de mañana que igual hace y igual no, y todo lo que no sea una cita fija.', items: { type: 'integer' } },
...
function planChatSystem(profile, ctx, previous, agenda, historial, parcial) {
  ...
  const estable = [
    ...
    "",
    "2.nonies. NO TODO VA A LA AGENDA.",
    "   La agenda es lo que TIENE que hacer ese día. Lo que puede que haga",
    "   no va ahí: va a \"Cardios guardados\", que es una estantería, no una",
    "   obligación. Se hace poniendo \"weekdays\" como lista VACÍA.",
    "   Van ahí:",
    "   · Las OPCIONES. Si el documento dice \"sábado: elige una\" con dos o",
    "     tres alternativas, NO elijas tú ni las pongas todas el sábado.",
    "     Guárdalas las dos y dile que el sábado escoge la que le apetezca.",
    "   · Lo de \"por si acaso\": un cardio de por la mañana que unos días",
    "     hace y otros no, una sesión suelta que quiere tener a mano.",
    "   · Lo que te pida guardar sin fecha.",
    "   Llenar la agenda de cosas opcionales la convierte en una lista de",
    "   tareas que no se cumple, y entonces deja de mirarla.",
    "",
    "2.pre. HAZLO. PREGUNTAR ES EL ÚLTIMO RECURSO.",
    "   Esta regla manda sobre todas las de abajo.",
    "   Si ya hay un plan y te piden un cambio —\"cambia el cardio a 30\",",
    "   \"mueve el jueves al viernes\", \"quita el martes\", \"el lunes más",
    "   corto\"— HAZLO Y YA. No devuelvas una pregunta.",
    "   Si falta un detalle, ELIGE TÚ lo más razonable, hazlo, y dilo en una",
    "   línea: \"te lo he puesto a las 19:00, dime si prefieres otra hora\".",
    "   Que un día se quede cargado o que dos cosas caigan juntas NO es",
    "   motivo para preguntar: lo resuelves, lo dices, y si no le vale te lo",
    "   corrige en el mensaje siguiente. Corregir cuesta una frase;",
    "   preguntar le cuesta un turno entero y la sensación de estar peleando",
    "   con la app.",
    "   Solo se pregunta cuando de verdad NO se puede decidir: te pide dos",
    "   cosas que se contradicen, o no se entiende de qué día habla.",
    "",
...
Responde en el idioma del usuario (por defecto español).`,
  ...
  try {
    ...
    // 4000 y no 2000.
    //
    // El marcador [CAMBIO: ...] que enciende el botón de aplicar va SIEMPRE
    // AL FINAL, así que es lo primero que se pierde si la respuesta se corta.
    // Medido: pedir "hazme los cardios con más sentido" gasta 1.775 tokens
    // de los 2.000 que había. Con la conversación un poco más larga o una
    // foto de por medio se pasaba, el marcador desaparecía, el botón no
